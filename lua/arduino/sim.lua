...
local function save_simulation_config(mcu, freq, fqbn)
  local path = get_config_path()
  if not path then return end
  
  -- Ensure build dir exists (it might not if we haven't compiled yet, 
  -- but we usually compile before running. We'll create it just in case.)
...
local function read_simulation_config()
  ...
  if data.mcu and data.freq then
    return data
  end
  return nil
end
...
local function launch_simavr(mcu, freq, elf_path)
  ...
  -- Ideally this should be in term.lua, but for now we implement it here
...
local function ensure_elf_and_run(mcu, freq)
  ...
  if not build_path then
     -- This is tricky if build_path isn't set. 
     -- But config.lua defaults to {project_dir}/build
     util.notify("Build path not configured.", vim.log.levels.ERROR)
     return
  end

  -- Find .elf file
  -- Strategy: Look for *.ino.elf in build path
  local elf_files = vim.fn.glob(build_path .. '/*.elf', true, true)
  local elf_file = nil
  if #elf_files > 0 then
    -- Prefer the one matching the sketch name if multiple (unlikely in build dir)
    -- Just take the first one for now or the most recent
    elf_file = elf_files[1]
  end

  local function run()
    -- Re-find elf in case it was just built
...
local function select_mcu_and_freq(callback)
  -- Get available cores from simavr
  -- simavr --list-cores
  -- Output is plain text list
...
local function setup_simavr()
  local fqbn = config.options.board
  -- Clean fqbn (remove options) for comparison/guessing
  ...
  if existing_config then
      if existing_config.fqbn then
          ...
          -- Legacy config, treat as valid to not annoy
          valid_config = true
      end
  end
  ...
  if guess then
      save_simulation_config(guess.mcu, guess.freq, base_fqbn)
      ensure_elf_and_run(guess.mcu, guess.freq)
  else
      select_mcu_and_freq(function(mcu, freq)
          save_simulation_config(mcu, freq, base_fqbn)
